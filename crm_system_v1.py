...
def render_order_management():
    ...
    with lcol:
        ...
        for _, o in order_summary.iterrows():
            ...
            st.markdown(f'**{o["model_name"]}** — {region}：待交付 **{int(o["待交付"])}** 台 / 库存 **{avail}** 台 → '
                        f'<span style="color:{color};font-weight:600;">{"盈余"+str(gap) if gap>=0 else "缺口"+str(abs(gap))} 台</span>', unsafe_allow_html=True)
    ...
    with rcol:
        ...
        st.markdown('<div style="background:#f8fafc;border-radius:10px;padding:14px;margin-bottom:8px;border:1px solid #e2e8f0;">', unsafe_allow_html=True)
        ...
        for region in ['重庆市', '四川省']:
            ...
            for _, r in region_leads.iterrows():
                ...
                st.markdown(f'{region} · {r["model_name"]}：潜客 {int(r["cnt"])} → 预计转化 **{predicted}** 单', unsafe_allow_html=False)
        st.markdown('</div>', unsafe_allow_html=True)
...
def factory_admin():
    ...
    QIANKE_SUBS     = ['🎯 潜客线索数据大屏', '🎯 线索抓取及分配', '⚙️ 评分规则配置', '📡 线索渠道效能分析看板', '❤️ 线索健康度看板', '📊 线索转化率看板']
    ...
    elif main_menu == '🎯 潜客运营' and sub_menu == '⚙️ 评分规则配置':
        st.header('AI评分规则自定义')
